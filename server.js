...
const app = express();
app.set('trust proxy', 1); // trust the nginx reverse proxy in front of this app
...
app.post('/api/auth/signup', signupLimiter, (req, res) => {
  ...
  const user = upsertUser({
    ...
    provider: 'password', role: 'student', approved: false, registeredAt: Date.now(),
    referredBy
  });
  req.session.user = { role: 'student', ...publicUser(user) };
  sendEmail({ to: user.email, subject: 'EduMitra — We received your registration', html: welcomeEmailHtml(user.name) });
...
app.post('/api/detect-role', (req, res) => {
  ...
  if (!key) return res.json({ role: 'unknown' });

  if (SUPERADMIN_EMAIL && key === SUPERADMIN_EMAIL) return res.json({ role: 'superadmin' });
  if (readAdmins()[key]) return res.json({ role: 'admin' });
  if (readCounsellors()[key]) return res.json({ role: 'counsellor' });
  if (readUsers()[key]) return res.json({ role: 'student' });
...
// Read-only counsellor list for any admin (not just superadmin) — used to populate
// the "assign counsellor" dropdown when approving students.
app.get('/api/admin/counsellors', requireAdmin, (req, res) => {
  const list = Object.values(readCounsellors()).map(({ passwordHash, google, ...safe }) => ({
    ...safe,
    calendarConnected: !!(google && google.refreshToken)
  }));
